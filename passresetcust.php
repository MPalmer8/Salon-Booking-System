...
if(isset($_POST['update'])){
    if($passwordval == "False"){
        ...
        if($matchpass == "False"){
            ...
            #This will update the passcode
            $hashed_passcode = password_hash($passcode, PASSWORD_BCRYPT);
            $sql = "UPDATE customer SET Passcode = '$hashed_passcode' WHERE CustomerID= '$id'";
            if($con->query($sql) === TRUE){
                echo "<div class='validated'>Successfully updated password</div>";
            }
            else{echo "<div class='validation'>Error: There was an error while resetting the password</div>";}
        }
    }
}
?>
